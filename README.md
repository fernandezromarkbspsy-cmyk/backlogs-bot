...
Go SeaTalk bot that watches `BAU Backlogs Summary!G8`. When the value changes, it waits 7 seconds for dependent sheet data to settle, then captures `Compliance Tracker!A1:X80`, renders it as an image, and sends an `@all` text update plus the report image to every known group.
...
When the bot is added to a SeaTalk group, the callback handler stores the `group_id` in `BAU Backlogs Summary!A2:A`. When the bot is removed, it removes that ID. A daily sync normalizes the sheet list by sorting and deduplicating known IDs.
