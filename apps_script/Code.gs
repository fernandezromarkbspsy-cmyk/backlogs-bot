const CONFIG = {
  ...
  BAU Backlogs Summary_TAB: 'BAU Backlogs Summary',
  BOT_NAME: 'Backlogs Bot',
...
function sendReportNow() {
  ...
  try {
    ...
    if (groupIds.length === 0) {
      throw new Error(`no SeaTalk group IDs found in ${CONFIG.BAU Backlogs Summary_TAB}!A2:A`);
    }
...
/**
 * Run daily from a time trigger to sort and deduplicate BAU Backlogs Summary!A2:A.
 */
...
function handleSeaTalkEvent_(event) {
  ...
  if (event.event_type === EVENT_BOT_ADDED_TO_GROUP_CHAT) {
    ...
    console.log(`stored group id ${groupId} in ${CONFIG.BAU Backlogs Summary_TAB}`);
    return;
  }
  ...
  if (event.event_type === EVENT_BOT_REMOVED_FROM_GROUP_CHAT) {
    ...
    console.log(`removed group id ${groupId} from ${CONFIG.BAU Backlogs Summary_TAB}`);
    return;
  }
...
function exportReportPdf_() {
  ...
  const filename = `backlogs-bot-${Utilities.formatDate(new Date(), CONFIG.TIMEZONE, 'yyyyMMdd-HHmmss')}.pdf`;
  return {
    filename: filename,
...
function botConfigSheet_() {
  const sheet = spreadsheet_().getSheetByName(CONFIG.BAU Backlogs Summary_TAB);
  if (!sheet) {
    throw new Error(`tab "${CONFIG.BAU Backlogs Summary_TAB}" not found`);
  }
  return sheet;
}
